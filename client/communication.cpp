    ...
    while (1)
        {
            ...
            if (read(commDescriptor, commBuffer, commBufferSize) <= 0)
                {
                    perror("[ERROR] : CANNOT READ DATA FROM DESCRIPTOR !\n");
                    perror(strerror(errno));
                    message.clear();
                    return message;
                }
...
Operation* Communication::recvOperationFromDescriptor()
{
    ...
    if (read(commDescriptor, &recvType, sizeof (TypeOfOp)) <= 0)  // Primim tipul
        {
            perror("[ERROR] : CANNOT READ DATA!\n");
            perror(strerror(errno));
            return op;
        }

    if (read(commDescriptor, &recvNoChars, sizeof (int)) <= 0)  // Primim numarul de caractere
        {
            perror("[ERROR] : CANNOT READ DATA!\n");
            perror(strerror(errno));
            return op;
    ...
    if (read(commDescriptor, &recvPosition, sizeof (int)) <= 0)  // Primim pozitia
        {
            perror("[ERROR] : CANNOT READ DATA!\n");
            perror(strerror(errno));
            return op;
